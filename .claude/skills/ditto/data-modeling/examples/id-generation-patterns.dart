...
// 1. ✅ UUID v4 (RECOMMENDED - general-purpose)
// 2. ✅ Auto-Generated (SIMPLEST - omit _id)
// 3. ✅ Composite Keys (ADVANCED - permission scoping)
// 4. ✅ ULID (TIME-ORDERED - chronological sorting)
// 5. ❌ Sequential IDs (ANTI-PATTERN - collision risk)
// 6. ✅ Human-Readable Display (UUID + display fields)
// 7. ✅ Migration from Sequential IDs (dual-write pattern)
//
// ============================================================================
...
/// ✅ GOOD: UUID v4 provides collision-free IDs for distributed systems
Future<void> uuidV4Pattern(Ditto ditto) async {
  print('✅ Pattern 1: UUID v4 (RECOMMENDED)\n');
  ...
  print('✅ Created order with UUID v4: $orderId');
...
/// ✅ GOOD: Omit _id to let Ditto auto-generate UUID (simplest approach)
Future<void> autoGeneratedPattern(Ditto ditto) async {
  ...
  print('✅ Created order with auto-generated ID');
...
/// ✅ GOOD: Composite keys for permission scoping and query optimization
Future<void> compositeKeyPattern(Ditto ditto) async {
  ...
  print('✅ Created order with composite key');
...
/// ✅ GOOD: ULID provides time-ordered, lexicographically sortable IDs
...
/// ❌ BAD: Sequential IDs cause collisions in distributed systems
Future<void> sequentialIdAntiPattern(Ditto? ditto) async {
  ...
  print('');
  print('❌ Problem: Collision in distributed systems');
  print('   Scenario:');
  ...
  print('   - Both devices sync → COLLISION → data loss or undefined behavior');
  print('');
  print('❌ Issues:');
  ...
  print('');
  print('✅ Solution: Use UUID v4, auto-generated, or ULID instead');
}
...
/// ✅ GOOD: Combine UUID (collision-free) with human-readable display fields
Future<void> humanReadableDisplayPattern(Ditto ditto) async {
  ...
  print('✅ Created order with human-readable display');
...
/// ✅ GOOD: Dual-write pattern for migrating from sequential to UUID IDs
Future<void> migrationDualWritePattern(Ditto ditto) async {
  ...
  print('✅ Migration: Dual-write pattern');
  ...
  print('✅ Query by new UUID: ${result.items.length} results');
  ...
  print('✅ Query by legacy ID: ${legacyResult.items.length} results');
  print('');
...
/// ❌ BAD: Timestamp-only IDs (no randomness)
Future<void> timestampOnlyAntiPattern(Ditto? ditto) async {
  print('\n❌ Anti-Pattern: Timestamp-Only IDs\n');

  // ❌ BAD: Timestamp-only (no randomness component)
  final timestampId = DateTime.now().millisecondsSinceEpoch.toString();

  print('Generated timestamp-only ID: $timestampId');
  print('');
  print('❌ Problem: Collisions within same millisecond window');
  print('   - Multiple writes within same millisecond → collision');
  ...
  print('');
  print('✅ Solution: Use UUID v4 or ULID (includes randomness)');
}

/// ❌ BAD: Counter-based IDs
Future<void> counterBasedAntiPattern(Ditto? ditto) async {
  print('\n❌ Anti-Pattern: Counter-Based IDs\n');

  // ❌ BAD: Counter-based ID generation
  var counter = 1;
  final counterId = 'product_$counter';

  print('Generated counter-based ID: $counterId');
  print('');
  print('❌ Problem: Requires centralized coordination');
  ...
  print('');
  print('✅ Solution: Use UUID v4 for independent ID generation');
}

/// ❌ BAD: Sequential displayId (no randomness)
Future<void> sequentialDisplayIdAntiPattern(Ditto? ditto) async {
  print('\n❌ Anti-Pattern: Sequential displayId (No Randomness)\n');

  // ❌ BAD: Sequential displayId generation
  final date = DateTime.now();
  final dateStr = '${date.year}${date.month.toString().padLeft(2, '0')}${date.day.toString().padLeft(2, '0')}';
  ...
  print('');
  print('❌ Problem: Poor UX when multiple devices create orders on same day');
  ...
  print('   - Both devices sync → Duplicate display IDs confuse users');
  print('');
  print('⚠️ NOTE: displayId is not the document ID (_id ensures uniqueness)');
  print('         Duplicate displayIds cause UX issues, not system errors');
  print('');
  print('✅ Solution: Use random suffix to reduce user confusion');
...
void printDecisionTree() {
  print('\n📋 Decision Tree: Choosing ID Generation Pattern\n');
  print('Need to generate document _id?');
  print('  ↓');
  print('Human-readable required for debugging?');
  print('  ↓ YES → Add display field alongside UUID');
  print('  |         (_id: UUID, displayId: "ORD-2025-042")');
  print('  ↓ NO');
  print('  ↓');
  print('Chronological sorting required?');
  print('  ↓ YES → Use ULID (time-ordered)');
  print('  ↓ NO');
  print('  ↓');
  print('Permission scoping needed?');
  print('  ↓ YES → Use Composite Keys');
  print('  ↓ NO');
  print('  ↓');
  print('Want simplest approach?');
  print('  ↓ YES → Omit _id (auto-generated)');
  print('  ↓ NO');
  print('  ↓');
  print('→ Use UUID v4 (general-purpose, recommended)');
}
...
Future<void> main() async {
  ...
  print('Summary:');
  print('=' * 80);
  print('✅ UUID v4: General-purpose, recommended (collision-free)');
  print('✅ Auto-Generated: Simplest (omit _id, Ditto auto-generates)');
  print('✅ Composite Keys: Permission scoping, query optimization');
  print('✅ ULID: Time-ordered sorting, chronological queries');
  print('❌ Sequential IDs: AVOID (collision risk in distributed systems)');
  print('❌ Timestamp-Only: AVOID (collisions within same millisecond)');
  print('❌ Counter-Based: AVOID (requires centralized coordination)');
  print('❌ Sequential displayId: AVOID (poor UX - consider random suffix)');
  print('=' * 80);
  print('');
  print('Key Takeaway:');
